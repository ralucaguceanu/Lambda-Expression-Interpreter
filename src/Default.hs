...
first = Abs "m" $ App vm (Abs "x" $ Abs "y" $ vx)
second = Abs "m" $ App vm (Abs "x" $ Abs "y" $ vy)
...
nPred = Abs "n" $ Abs "f" $ Abs "x" $ App (App (App vn (Abs "g" $ Abs "h" $ App vh (App vg vf))) (Abs "u" vx)) (Abs "u" (Var "u"))
